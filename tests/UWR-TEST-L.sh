...
mkdir -p $DPATH/Writer;
mkdir -p $DPATH/Reader;
NUM=$(getmax.sh -s "$NUMSET"); RK=8192; INTV=0;
RSIZE=$(($RK * 1024));
SIZE=$(($MAXTPUT * 1024 * 1024 * $PERIOD));
TAG="${NUM}T_${INTV}INT_${RK}K";
echo "TAG=$TAG; SIZE=$SIZE; NUM=$NUM; RSIZE=$RSIZE; DPATH=$DPATH/Reader";
./SimpleWriterBatchTest.sh $TAG $SIZE $RSIZE $NUM $DPATH/Reader > /dev/null;
...
for INTV in $INTVSET; do
	for NUM in $NUMSET; do
		for RK in $RSET; do
			...
			./UnlimitedWriterReaderBatchTest.sh $TAG $PERIOD $NUM $INTV $RSIZE $DPATH > $DRES/$TAG;
		done
	done
done